#!/usr/bin/env python3
"""
Audio monitoring system using SOLID principles.

This module provides:
- Main monitoring loop (run_monitor) using dependency injection
- Utility functions (dbfs, constants) used by other modules
"""
import datetime
...
def start_arecord(config: Dict[str, Any]) -> subprocess.Popen:
    """
    Start arecord process for audio capture.
    
    This function is kept for backward compatibility with baseline.py and other scripts.
    For new code, use AudioCapture class from core.audio instead.
    """
    audio = config["audio"]
    device = audio["device"]
    sample_rate = audio["sample_rate"]
    channels = audio["channels"]
    # Use sample_format if available, otherwise format (for backward compatibility)
    format_type = audio.get("sample_format", audio.get("format", "S16_LE"))
...
def _print_metrics(chunk, baseline_tracker, in_event: bool) -> None:
    """Print live monitoring metrics."""
    peak_db = dbfs(chunk.peak)
    rms_db = dbfs(chunk.rms)
